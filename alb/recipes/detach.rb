...
ruby_block "detach from ALB" do
  block do
    ...
    ssl_redirect_target_group_arn = node[:alb_helper][:ssl_redirect_target_group_arn]
    ...
    unless ssl_redirect_target_group_arn.nil?
      ssl_redirect_target_to_detach = {
        target_group_arn: ssl_redirect_target_group_arn,
        targets: [ { id: ec2_instance_id } ],
      }

      Chef::Log.info("Deregistering EC2 instance #{ec2_instance_id} from Target Group #{ssl_redirect_target_group_arn}")
      client.deregister_targets(ssl_redirect_target_to_detach)
    end
