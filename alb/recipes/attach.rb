...
ruby_block "attach to ALB" do
  block do
    ...
    unless ssl_redirect_target_group_arn.nil?
      Chef::Log.info("Registering EC2 instance #{ec2_instance_id} with target group #{ssl_redirect_target_group_arn}")

      ssl_redirect_target_to_attach = {
        target_group_arn: ssl_redirect_target_group_arn,
        targets: [{ id: ec2_instance_id }]
      }

      client.register_targets(ssl_redirect_target_to_attach)
    end
  end
  action :run
end
